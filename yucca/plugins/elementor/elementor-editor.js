...
jQuery( document ).ready(
	function() {
		...
		yucca_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="yucca_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.yucca_post_editor' );
						if ( link.length > 0 ) {
							yucca_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.yucca_post_editor', function(e) {
						yucca_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('yucca_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function yucca_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
