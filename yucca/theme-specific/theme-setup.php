...
if ( ! defined( 'YUCCA_THEME_FREE' ) ) {
	define( 'YUCCA_THEME_FREE', false );
}
if ( ! defined( 'YUCCA_THEME_FREE_WP' ) ) {
	define( 'YUCCA_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'YUCCA_THEME_ALLOW_UPDATE' ) ) {
	define( 'YUCCA_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'YUCCA_ALLOW_SKINS' ) ) {
	define( 'YUCCA_ALLOW_SKINS', true );
}
if ( ! defined( 'YUCCA_DEFAULT_SKIN' ) ) {
	define( 'YUCCA_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'YUCCA_REMEMBER_SKIN' ) ) {
	define( 'YUCCA_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'yucca_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function yucca_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = yucca_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'yucca_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'yucca_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function yucca_trx_updater_get_theme_info( $info ) {
		$engine = yucca_storage_get( 'engine' );
...
if ( ! function_exists( 'yucca_original_theme_slug' ) ) {
	add_filter( 'yucca_filter_original_theme_slug', 'yucca_original_theme_slug' );
	/**
...
if ( ! function_exists( 'yucca_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'yucca_importer_set_options', 9 );
	/**
	 ...
	 */
	function yucca_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( yucca_get_protocol() . ':' . yucca_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( yucca_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'yucca_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'yucca_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function yucca_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( yucca_get_protocol() . ':' . yucca_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'yucca_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'yucca_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function yucca_customizer_theme_setup1() {
		yucca_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'yucca_filter_allow_fullscreen' return false)
