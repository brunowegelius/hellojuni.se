...
if ( ! function_exists( 'yucca_storage_get' ) ) {
	/**
	 ...
	 */
	function yucca_storage_get( $var_name, $default = '' ) {
		global $YUCCA_STORAGE;
		return isset( $YUCCA_STORAGE[ $var_name ] ) ? $YUCCA_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'yucca_storage_set' ) ) {
	/**
	 ...
	 */
	function yucca_storage_set( $var_name, $value ) {
		global $YUCCA_STORAGE;
		$YUCCA_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'yucca_storage_empty' ) ) {
	/**
	 ...
	 */
	function yucca_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $YUCCA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $YUCCA_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $YUCCA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'yucca_storage_isset' ) ) {
	/**
	 ...
	 */
	function yucca_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $YUCCA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $YUCCA_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $YUCCA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'yucca_storage_unset' ) ) {
	/**
	 ...
	 */
	function yucca_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $YUCCA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $YUCCA_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $YUCCA_STORAGE[ $var_name ] ) ) {
				unset( $YUCCA_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'yucca_storage_inc' ) ) {
	/**
	 ...
	 */
	function yucca_storage_inc( $var_name, $value = 1 ) {
		global $YUCCA_STORAGE;
		if ( empty( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = 0;
		}
		$YUCCA_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'yucca_storage_concat' ) ) {
	/**
	 ...
	 */
	function yucca_storage_concat( $var_name, $value ) {
		global $YUCCA_STORAGE;
		if ( empty( $YUCCA_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'yucca_storage_get_array' ) ) {
	/**
	 ...
	 */
	function yucca_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $YUCCA_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $YUCCA_STORAGE[ $var_name ][ $key ] ) ? $YUCCA_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'yucca_storage_set_array' ) ) {
	/**
	 ...
	 */
	function yucca_storage_set_array( $var_name, $key, $value ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $YUCCA_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$YUCCA_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'yucca_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function yucca_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
			$YUCCA_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$YUCCA_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$YUCCA_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'yucca_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function yucca_storage_merge_array( $var_name, $key, $value ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$YUCCA_STORAGE[ $var_name ] = array_merge( $YUCCA_STORAGE[ $var_name ], $value );
		} else {
			$YUCCA_STORAGE[ $var_name ][ $key ] = array_merge( $YUCCA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'yucca_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function yucca_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			yucca_array_insert_after( $YUCCA_STORAGE[ $var_name ], $after, $key );
		} else {
			yucca_array_insert_after( $YUCCA_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'yucca_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function yucca_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			yucca_array_insert_before( $YUCCA_STORAGE[ $var_name ], $before, $key );
		} else {
			yucca_array_insert_before( $YUCCA_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'yucca_storage_push_array' ) ) {
	/**
	 ...
	 */
	function yucca_storage_push_array( $var_name, $key, $value ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $YUCCA_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
				$YUCCA_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $YUCCA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'yucca_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function yucca_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $YUCCA_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $YUCCA_STORAGE[ $var_name ] ) && is_array( $YUCCA_STORAGE[ $var_name ] ) && count( $YUCCA_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $YUCCA_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $YUCCA_STORAGE[ $var_name ][ $key ] ) && is_array( $YUCCA_STORAGE[ $var_name ][ $key ] ) && count( $YUCCA_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $YUCCA_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'yucca_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function yucca_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
			$YUCCA_STORAGE[ $var_name ][ $key ] = 0;
		}
		$YUCCA_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'yucca_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function yucca_storage_concat_array( $var_name, $key, $value ) {
		global $YUCCA_STORAGE;
		if ( ! isset( $YUCCA_STORAGE[ $var_name ] ) ) {
			$YUCCA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $YUCCA_STORAGE[ $var_name ][ $key ] ) ) {
			$YUCCA_STORAGE[ $var_name ][ $key ] = '';
		}
		$YUCCA_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'yucca_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function yucca_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $YUCCA_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $YUCCA_STORAGE[ $var_name ] ) ? $YUCCA_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $YUCCA_STORAGE[ $var_name ] ) ? $YUCCA_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'yucca_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function yucca_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $YUCCA_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $YUCCA_STORAGE[ $var_name ]->$prop ) ? $YUCCA_STORAGE[ $var_name ]->$prop : $default;
	}
}
