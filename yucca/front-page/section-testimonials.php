<div class="front_page_section front_page_section_testimonials<?php
	$yucca_scheme = yucca_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $yucca_scheme ) && ! yucca_is_inherit( $yucca_scheme ) ) {
		echo ' scheme_' . esc_attr( $yucca_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( yucca_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( yucca_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$yucca_css      = '';
		$yucca_bg_image = yucca_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$yucca_anchor_icon = yucca_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$yucca_anchor_text = yucca_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( yucca_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$yucca_css      = '';
			$yucca_bg_mask  = yucca_get_theme_option( 'front_page_testimonials_bg_mask' );
			$yucca_bg_color_type = yucca_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $yucca_bg_color_type ) {
				$yucca_bg_color = yucca_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$yucca_caption = yucca_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $yucca_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $yucca_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $yucca_caption, 'yucca_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$yucca_description = yucca_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $yucca_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $yucca_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $yucca_description ), 'yucca_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! yucca_exists_trx_addons() ) {
						...
						yucca_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
