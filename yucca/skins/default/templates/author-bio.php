...
?>>
	...
	<div class="author_description">
		...
		?>><a class="author_link fn" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><?php
			the_author();
		?></a></h5>
		<div class="author_label"><?php esc_html_e( 'About Author', 'yucca' ); ?></div>
		<div class="author_bio"<?php
			if ( yucca_is_on( yucca_get_theme_option( 'seo_snippets' ) ) ) {
				?> itemprop="description"<?php
			}
		?>>
			<?php echo wp_kses( wpautop( get_the_author_meta( 'description' ) ), 'yucca_kses_content' ); ?>
			<div class="author_links">
				<?php do_action( 'yucca_action_user_meta', 'author-bio' ); ?>
			</div>
		</div>
