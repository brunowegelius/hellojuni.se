...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			yucca_create_observer( 'yucca-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				yucca_gutenberg_editor_init();
			} );
		} else {
		 	yucca_gutenberg_editor_init();
		}
	...
	function yucca_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'yucca_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'yucca_inited' ) ) {
	...
	// Init on page load
	function yucca_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = yucca_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + YUCCA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + YUCCA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in YUCCA_STORAGE['override_classes'] ) {
				$obj.addClass( YUCCA_STORAGE['override_classes'][i].replace( '%s', YUCCA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( yucca_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && YUCCA_STORAGE['sidebar_position'] != 'hide' ) {
				if ( yucca_apply_filters( 'yucca_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'yucca_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( yucca_apply_filters( 'yucca_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				yucca_remove_observer( 'yucca-check-editor-styles-wrapper' );
				yucca_create_observer( 'yucca-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'yucca_inited' );
	}
} );
